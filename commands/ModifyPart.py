...
def defineParameterManagers():
    ...
    def setInsertLightBulb(comp, insertType, isOn):
        for i in range(comp.occurrences.count):
            ...
            if subComp.attributes.itemByName('fetchParams', 'part_data') and 'identifiers' in fetchParams.getPartData(subComp):
                identifiers = fetchParams.getPartData(subComp)['identifiers']
    ...
    class DistanceModule(ParameterManager):
        ...
        def show(self, occ):
            comp = occ.component
            ao = apper.AppObjects()
            parameter = fetchParams.getPartData(comp)['parameters'][self.parameterId]
...
def parameterManagersInParameters(comp):
    parameters = fetchParams.getPartData(comp)['parameters']
...
class ModifyPart(apper.Fusion360CommandBase):
    ...
    def on_input_changed(self, command: adsk.core.Command, inputs: adsk.core.CommandInputs, changed_input, input_values):
        ...
        if selectionInput.selectionCount > 0:
            ...
            for i in range(2):
                if selectedComp.attributes.itemByName('fetchParams', 'part_data') and 'parameters' in fetchParams.getPartData(selectedComp):
    ...
    def on_create(self, command: adsk.core.Command, inputs: adsk.core.CommandInputs):
        ...
        global importedPart
        if importingPart:
            # if importedPart.attributes.itemByName('fetchParams', 'part_data'):
            # importedCompAttributes = fetchParams.getPartData(importedPart)
    ...
    def command_event_received(self, event_args, command_id, command_definition):
        ...
        if command_id == 'FusionMoveCommand':
            if importingPart:
                ...
                if tempPart.component.attributes.itemByName('fetchParams', 'part_data') and 'parameters' in fetchParams.getPartData(tempPart.component):
                    global importedPart
